...
uvicorn --factory archivepodcast.run_webapp:create_app --port 5100 --reload   # dev server
...
- **Module-level singletons** in [src/archivepodcast/instances/](src/archivepodcast/instances/): `health`, `event_times` (profiler), `get_app_paths()` (path helper — must be first called with `instance_path`, later calls take no args), `local_file_cache`/`s3_file_cache`, `get_ap_config()`, and the live `PodcastArchiver` instance. Routers and archiver code import these globals rather than passing state around.
